...
class StaticCreditController extends GetxController {
  ...
  void dataChartWeekly() async {
    flspot = [];
    for (var i = 0; i < getNormalizeWeekTransactions.length; i++) {
      flspot.add(
          FlSpot(double.parse(i.toString()), getNormalizeWeekTransactions[i]));
    }
  }

  void dataChartMonthly() async {
    flspotMonth = [];
    for (var i = 0; i < getNormalizeMonthTransactions.length; i++) {
      flspotMonth.add(
          FlSpot(double.parse(i.toString()), getNormalizeMonthTransactions[i]));
    }
  }
